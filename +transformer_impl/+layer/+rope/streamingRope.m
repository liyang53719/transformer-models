function [outBeat, outValid, busy, done] = streamingRope(start, cfgNumTokens, cfgNumHeads, inBeat, inValid)
% streamingRope   Streaming 256-bit RoPE accelerator for Qwen2.5-1.5B prefill.
%
%   The accelerator accepts one 256-bit beat per cycle as an 8-lane single
%   vector. Input ordering matches the projection outputs reshaped as
%   [headDim, numHeadsThisStream, numTokens], flattened in MATLAB column-major
%   order. For each head, the first 8 beats are the real half and the next 8
%   beats are the imaginary half used by RoPE.
%
%   The design keeps throughput at one beat per cycle after pipeline fill.
%   It uses only two 8-beat local buffers:
%   1. A first-half input buffer for the current head.
%   2. A second-half output buffer for the previous head.
%
%   Token-to-token phase updates are done recursively from token p to p+1, so
%   the implementation does not need a large sin/cos SRAM indexed by token.
%
%   Inputs:
%       start       - Pulse to start a new prefill run.
%       cfgNumTokens- Number of tokens in this run, 64 to 1024.
%       cfgNumHeads - Number of heads in this stream. Use 2 for K or 12 for Q.
%       inBeat      - 1-by-8 single beat.
%       inValid     - Input beat valid.
%
%   Outputs:
%       outBeat     - 1-by-8 single beat after RoPE.
%       outValid    - Output beat valid.
%       busy        - Accelerator busy.
%       done        - One-cycle pulse when the final beat is emitted.

%#codegen

persistent firstHalfBuf secondHalfBuf currentCos currentSin deltaCos deltaSin
persistent runActive ingestDone drainActive drainIndex
persistent activeTokenCount activeHeadCount activeBeatsPerToken
persistent activeTokenIndex beatInToken remainingInputBeats constantsReady
...
if runActive
    if inValid && ~ingestDone
        ...
        if beatInHead < 8
            ...
            if drainActive
                outBeat = secondHalfBuf(drainIndex + uint8(1), :);
                outValid = true;
                if drainIndex == 7
                    drainActive = false;
                    drainIndex = uint8(0);
            ...
            rowIndex = beatInHead - uint8(7);
            firstHalfBeat = firstHalfBuf(rowIndex, :);
            cosBeat = currentCos(rowIndex, :);
            sinBeat = currentSin(rowIndex, :);

            [outBeat, secondHalfBuf(rowIndex, :)] = iRotateBeat(firstHalfBeat, inBeat, cosBeat, sinBeat);
            outValid = true;
